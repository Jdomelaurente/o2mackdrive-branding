...
There is **no backend or database** — all data (cars, services, FAQs, testimonials) is hardcoded in `src/features/*/data/` and `src/shared/data/`. It is a pure frontend application.
...
### Logo
- **Primary logo:** `public/logo.png` — used in the navbar, footer, and BrandLogo component
- **Icon/favicon:** `public/favicon-64.png` — site icon and Apple touch icon
- The `BrandLogo` component (`src/shared/components/ui/BrandLogo.tsx`) renders the logo with three size variants: sm (208px), md (320px), lg (384px)
