...
export async function PATCH(
  ...
  try {
    ...
    const result = await query(
      "UPDATE sell_trade_inquiries SET status = $2 WHERE id = $1 RETURNING id, status",
    ...
    console.error("PATCH /api/admin/inquiries/sell-trade/[id]", err);
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    const result = await query("DELETE FROM sell_trade_inquiries WHERE id = $1", [Number(id)]);
    if (result.rowCount === 0) {
      return Response.json({ error: "Inquiry not found." }, { status: 404 });
    }
    return Response.json({ ok: true });
  } catch (err) {
    console.error("DELETE /api/admin/inquiries/sell-trade/[id]", err);
